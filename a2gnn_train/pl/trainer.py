...
from a2gnn import models
# from GMcolor import models
from a2gnn.data.dataset_loaders import init_data_loader
from a2gnn.utils.loss import compute_loss
from a2gnn.utils.metrics import compute_metrics_batch
...
class MatcherTrainer(pl.LightningModule):
    ...
    def parse_data(self, data):
        inputs = [data["pts2dm"], data["idx2d"], data["pts3dm"], data["idx3d"], data["color2d"], data["color3d"]]
        return inputs
    ...
    def training_step(self, data, batch_idx):
        ...
        # if batch_idx in range(2795, 2805):  # Close inspection around the problematic batch
        #     print(f"Batch {batch_idx}: X={x}, Y={y}, Y_hat={y_hat}")
        # if torch.isnan(loss):
        #     print(f'NaN Loss at batch index: {batch_idx}, X={x}, Y={y}, Y_hat={y_hat}')
    ...
    #             print(f'NaN gradient in {name} at step {self.global_step}')
...
def train(args, exp_name):
    ...
    # train_loader_sift = init_data_loader(args, split=args.train_split)
    # train_loader_superpoint = init_data_loader(args, split=args.train_split)
    # train_loader_r2d2 = init_data_loader(args, split=args.train_split)
    # train_loader = ConcatDataset([train_loader_sift, train_loader_superpoint, train_loader_r2d2])
