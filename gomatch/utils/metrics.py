...
def compute_metrics_sample(
    ...
    gt,_ = torch.where(matches_gt_)
    # Pose estimation
    # i3d, i2d = torch.where(matches_est_)
    # inl = torch.sum( torch.any(i3d.cpu().unsqueeze(1) == i3d_gt.unsqueeze(0), dim=1) )
    ...
    # tp = torch.sum(matches_est_.cpu() & matches_gt_)
    # pgt = torch.sum(matches_gt_)
    # pest = torch.sum(matches_est_.cpu())
    # # recall = tp / pgt if pgt > 0.0 else pgt.float()
    # precision = tp / pest if pest > 0.0 else pest.float()
    # set_i3d = set(i3d.cpu().tolist())
    # set_i3d_gt = set(i3d_gt.tolist())
    # intersection3d = set_i3d.intersection(set_i3d_gt)
    # set_i2d = set(i2d.cpu().tolist())
    # set_i2d_gt = set(i2d_gt.tolist())
    # intersection2d = set_i2d.intersection(set_i2d_gt)
    # matches_est_flat = matches_est_.view(-1)
    # matches_gt_flat = matches_gt_.view(-1)
    # intersection = torch.cat([matches_est_flat, matches_gt_flat]).unique(return_counts=True)
    # ntersection = intersection[0][intersection[1] > 1]

    # i3d_gt, i2d_gt = torch.where( matches_gt_ & matches_est_.cpu() )

    metrics["reproj_gt3d_estpose"].append(reproj_errs)
    metrics["gt"].append(len(gt))
    # metrics["matched_2d"].append( pts2d_pix[i2d, :2].cpu().numpy() )
    # metrics['matched_3d'].append( pts3d[i3d].cpu().numpy() )
    # metrics['3d_gt'].append(pts3d[i3d_gt].cpu().numpy() )
    # metrics['2d_gt'].append(pts2d_pix[i2d_gt, :2].cpu().numpy())
    # metrics["inliers"].append(inl.cpu().numpy() )
