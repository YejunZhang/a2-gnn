...
def prepare_sample_inputs(
    pts2d: TensorOrArray, pts3d: TensorOrArray,color2d, color3d, device: Device
    ...
    if isinstance(color2d, np.ndarray):
        pts2d = torch.from_numpy(color2d)
    if isinstance(color3d, np.ndarray):
        pts3d = torch.from_numpy(color3d)

    pts2d = pts2d.to(device)
    pts3d = pts3d.to(device)
    color2d = color2d.to(device)
    color3d = color3d.to(device)
    idx2d = torch.full((len(pts2d),), 0, device=device)
    idx3d = torch.full((len(pts3d),), 0, device=device)
    return pts2d, idx2d, pts3d, idx3d, color2d, color3d
...
class GenericModelWrapper(torch.nn.Module):
    ...
    def forward(
        ...
        return self.matcher(pts2d, idx2d, pts3d, idx3d, color2d, color3d)
...
class MatcherEvaluator:
    ...
    def match_sample(
        self, pts2dm: TensorOrArray, pts3dm: TensorOrArray, color2d: TensorOrArray, color3d: TensorOrArray
    ) -> Tuple[np.ndarray, np.ndarray]:
        inputs = prepare_sample_inputs(pts2dm, pts3dm, color2d, color3d, self.device)
        t0 = time.time()
        assert (
    ...
    def match_sample_separate_covis(
        ...
        for pts3d_ids, pts3dm_ids in zip(covis_pts3d_ids, covis_pts3dm_ids):
            ...
            icolor3d = color3d[pts3dm_ids]
            imatches_est, iscores = self.match_sample(pts2dm, ipts3dm, color2d, icolor3d)
    ...
    def eval_batch_merge_before_match(
        ...
        if not self.oracle:
            ...
            inputs = self.list2device(
                [data["pts2dm"], data["idx2d"], data["pts3dm"], data["idx3d"], data["color2d"], data["color3d"]]
            )
    ...
    def eval_batch_merge_after_match(
        ...
        for bid in bids:
            ...
            if self.oracle:
                ...
                color2d = data["color2d"][mask2d]
                color3d = data["color3d"][mask3dm]
                ...
                matches_est = self.match_sample_separate_covis(
                    pts2dm, pts3dm, pts3d, color2d, color3d, unmerge_mask, covis_ids
                )
            ...
            # matched_2d = pts2d[i2d, :2]  # Extract matched 2D points
            # matched_3d = pts3d[i3d]
            
            # metrics["matched_2d"].append(matched_2d.cpu().numpy())
            # metrics["matched_3d"].append(matched_3d.cpu().numpy())
            self.metrics["name"].append(data["name"][bid]) 
            # self.metrics["matches_gt"].append(matches_gt)
            # self.metrics["matches_est"].append(matches_est)
