...
# def get_graph_feature1(
#     coords: torch.Tensor, feats: torch.Tensor, k: int = 10,ind = None
# ) -> torch.Tensor:
#     """
#     Apply KNN search based on coordinates, then concatenate the features to the centroid features
#     Input:
...
#         x2 = get_graph_feature1(coords, x1.squeeze(-1), self.k,ind=x1_old_ind)
#         x2 = self.annuconv2(x2)
#         # ang++ 
#         nei2 = torch.gather( coords1,dim=-1, index= x1_old_ind.unsqueeze(1).repeat(1,2,1,1))
#         ang2 = cosine_similarity(coords.unsqueeze(-1).repeat(1, 1, 1, 9),nei2)
#         f_ang2 = self.angle_enc1(ang2)
#         # final update
#         x3 = torch.cat((x0, x1+f_ang1, x2+f_ang2), dim=1) # 1, 512, 805, 1
...
#         x2_old = get_graph_feature1(coords, x1_old.squeeze(-1), self.k,ind=x1_old_ind)
...
class SelfAttention(nn.Module):
    def __init__(self, feature_dim: int, k: int = 10) -> None:
        ...
        # self.ang_mlp = self.expander1 = nn.Sequential(
        #     nn.Conv2d(in_channels=1, out_channels=128, kernel_size=(1, 1)),
