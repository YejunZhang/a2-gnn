...
class SelfAttention1(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, features: torch.Tensor) -> torch.Tensor:
        ...
        # global space
        w_p1 = self.mlp1(x1).view(B, -1) #[1,805]
        out_gs1 = self.gcn1(x1, w_p1.detach()) #[1, 128, 805, 1]
        x25_ind = knn(out_gs1.squeeze(-1), k=self.k)
        x25_nei = get_graph_feature1(x1,k=self.k,idx=x25_ind)
        ...
        x3 = torch.cat((x0, x1, x25), dim=1) # 1, 512, 805, 1
