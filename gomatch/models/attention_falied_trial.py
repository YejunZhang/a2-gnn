...
# from .net_modules import PointResNetEncoder
...
def get_graph_feature(
    ...
    idx = idx[:, :, 1:]  # [B, N, K] /// torch.Size([1, 805, 9])
    return_idx = idx.clone() # /// torch.Size([1, 805, 9])
    # import pdb; pdb.set_trace()
    idx = idx.unsqueeze(1).repeat(1, C, 1, 1)  # [B, C, N, K] /// torch.Size([1, 128, 805, 9])
...
class SelfAttention(nn.Module):
    def __init__(self, feature_dim: int, k: int = 10) -> None:
        ...
        self.annuconv1 = nn.Sequential(
                ...
                nn.Conv2d(self.in_channel, self.in_channel, (1, 3)), # change the mlp here?
                nn.InstanceNorm2d(self.in_channel),
                nn.ReLU(inplace=True),
            )
        ...
        self.k = k
        # +++++
        # self.mlp = nn.Conv2d(in_channels=1,out_channels=128,kernel_size=1,bias=False)
        # self.mlp_in = nn.InstanceNorm2d(feature_dim)
        self.mlp = MLPWithInstanceNorm()
        self.mlp2 = MLPWithInstanceNorm2()

        # self.q = feature_dim #128
    ...
    def forward(self, coords: torch.Tensor, features: torch.Tensor) -> torch.Tensor:
        ...
        # import pdb; pdb.set_trace()
        x0 = features.unsqueeze(-1)  # [B, C, N, 1]                                         torch.Size([1, 128, 805, 1])
        # import pdb; pdb.set_trace()
        ...
        x1_old,x1_old_ind = get_graph_feature(coords, x0.squeeze(-1), self.k)                # ind: torch.Size([1, 805, 9])

        # x1 = get_graph_feature1(x0,k=self.k) # torch.Size([1, 256, 805, 9])                 torch.Size([1, 128, 805, 9])
        x1 = self.annuconv1(x1_old) # torch.Size([1, 128, 805, 1])                              torch.Size([1, 128, 805, 1])

        x2,_ = get_graph_feature(coords, x1.squeeze(-1), self.k)
        # x2 = get_graph_feature1(x1,k=self.k) # torch.Size([1, 256, 805, 9])
        ...
        ############# old part 
        # x1_old = get_graph_feature(coords, x0.squeeze(-1), self.k)                  # 1,128,805,9
        x1_old = F.leaky_relu(self.in1(self.conv1(x1_old)), negative_slope=0.2)       # 1,128,805,9
        # x1_old = x1_old.max(dim=-1,keepdim=True)[0]                                 # 1,128,805,1        
        x1_old, x1_max1_ind = x1_old.max(dim=-1, keepdim=True)
        unique_elements, counts = torch.unique(x1_max1_ind.squeeze(), return_counts=True,dim=0)
        most_frequent = unique_elements[torch.argmax(counts)]  # torch.Size([805])
        nei = x1_old_ind.squeeze()[torch.arange(x1_old_ind.size(1)),most_frequent] # torch.Size([805, 9])
        # import pdb; pdb.set_trace()
        nei_coord = coords.squeeze()[:,nei]
        ang1 = cosine_similarity(coords.squeeze(),nei_coord)
        # import pdb; pdb.set_trace()
        f_ang1 = self.mlp(ang1).reshape(1,128,-1,1)
        ...
        x2_old, x2_max1_ind = x2_old.max(dim=-1, keepdim=True)

        unique_elements2, counts2 = torch.unique(x2_max1_ind.squeeze(), return_counts=True,dim=0)
        most_frequent2 = unique_elements2[torch.argmax(counts2)]  # torch.Size([805])
        nei2 = x2_old_ind.squeeze()[torch.arange(x2_old_ind.size(1)),most_frequent2] # torch.Size([805, 9])
        nei_coord = coords.squeeze()[:,nei2]
        ang2 = cosine_similarity(coords.squeeze(),nei_coord)
        f_ang2 = self.mlp2(ang2).reshape(1,256,-1,1)
        # import pdb; pdb.set_trace()
        x3_old = torch.cat((x0, x1_old+f_ang1, x2_old+f_ang2), dim=1)
...
def cosine_similarity(tensor_a, tensor_b):
    ...
    dot_product = (tensor_a * tensor_b).sum(dim=0)
    
    # Calculate the norm of each tensor along the 0th dimension
    norm_a = tensor_a.norm(dim=0)
    norm_b = tensor_b.norm(dim=0)
...
class MLPWithInstanceNorm(nn.Module):
    def __init__(self):
        super(MLPWithInstanceNorm, self).__init__()
        # Define the architecture
        self.layer1 = nn.Linear(1, 32)  # Input layer
        self.norm1 = nn.InstanceNorm1d(32)
        self.layer2 = nn.Linear(32, 64)
        self.norm2 = nn.InstanceNorm1d(64)
        self.layer3 = nn.Linear(64, 128)
        self.norm3 = nn.InstanceNorm1d(128)
        self.layer4 = nn.Linear(128, 128)
        self.norm4 = nn.InstanceNorm1d(128)
        self.layer5 = nn.Linear(128, 128)  # Output layer

    def forward(self, x):
        # Forward pass through the network
        x = x.unsqueeze(2)  # Adjust shape for InstanceNorm1d
        ...
        x = self.layer5(x)
        x = x.squeeze(2)  # Remove the extra dimension added for normalization
        return x
    
class MLPWithInstanceNorm2(nn.Module):
    def __init__(self):
        super(MLPWithInstanceNorm2, self).__init__()
        # Define the architecture
        self.layer1 = nn.Linear(1, 32)  # Input layer
        self.norm1 = nn.InstanceNorm1d(32)
        self.layer2 = nn.Linear(32, 64)
        ...
        self.layer4 = nn.Linear(128, 128)
        self.norm4 = nn.InstanceNorm1d(128)
        self.layer5 = nn.Linear(128, 256)  # Output layer adjusted to 256
