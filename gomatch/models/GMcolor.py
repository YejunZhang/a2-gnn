...
class OTMatcher(nn.Module):
    def __init__(
        ...
        # 2D encoder
        self.kp2d_enc = PointResNetEncoder(in_channel=2, feat_channel=kp_feat_dim,num_layers=12)#12-->16 as more input data
        ...
        if share_kp2d_enc and p3d_dim == 2:
            ...
            self.kp3d_enc = PointResNetEncoder(
                in_channel=p3d_dim, feat_channel=kp_feat_dim,num_layers=12
            )
        ...
        # pos encoder
        # self.pos_enc = PointResNetEncoder(in_channel=2, feat_channel=kp_feat_dim,num_layers=12)
    ...
    def encode_pts(
        self,
        pts2d: torch.Tensor, ## [6551,2]
        idx2d: torch.Tensor, ## [6551]
        pts3d: torch.Tensor, 
        idx3d: torch.Tensor,
        color2d: torch.Tensor, ## [6551,3]
        color3d: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        # import pdb; pdb.set_trace()
        
        # pts2d = torch.cat((pts2d,color2d),dim=1) # [6551,5]
        # pts3d = torch.cat((pts3d,color3d),dim=1) # 
        
        # desc2d: [6551,128] desc3d: [3018,128]
        desc2d = self.kp2d_enc(pts2d, idx2d) + self.color_enc(color2d, idx2d) ## add key points feature and RGB feature into kinda of descriptors
        desc3d = self.kp3d_enc(pts3d, idx3d) + self.color_enc(color3d, idx3d) ## dim for 2d:[6551,128]//3d:[3018,128]  
...
class OTMatcherCls(nn.Module): #Cl means classifier
    def __init__(
        self,
        p3d_type: str,
        kp_feat_dim: int = 128,
