...
def load_scene_data(
    data_file: PathT,
    scenes: Collection[str], # like 0000,0001...
    scene3d_file: PathT, # the path is desc_cache train/test/val npy
    feature_dir: PathT, # the path is desc_cache sift1024
    ...
    for sid in scenes:
        ...
        for qid, im in scene_ims.items():
            ...
            pts2d, pts2d_uids = np.unique(pts2d, axis=0, return_index=True) #return unique element and index
            im.kpts = pts2d
            ...
            # dict_im = vars(im)
            # dict_im["color"] = kpts_color
            # im = Namespace(**dict_im)
...
def collect_covis_p3d_data(
    ...
    if pts3d:  # This checks if pts3d is not empty
        pts3d = np.concatenate(pts3d)
    else:
        # Handle the empty case appropriately, e.g., by initializing pts3d to a default value
        pts3d = np.array([]) 
